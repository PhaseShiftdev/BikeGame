...
[RequireComponent(typeof(HollyPlayerInputController))]
public class HollyPlayerMachine : SuperStateMachine {
	...
	private HollyPlayerInputController input;

	void Start () {
		// Put any code here you want to run ONCE, when the object is initialized

		input = gameObject.GetComponent<HollyPlayerInputController>();
	...
	void update(){
		...
		if (controller.currentGround.IsGrounded (true, 0.5f)) {
			walking = false;
		}
	}
